...
//     private static String WSS = "wss://47.254.34.146/wss";
    //本地测试信令地址
    private static String WSS = "ws://192.168.1.24:3000";
    ...
    public static void sendMsg(MeetingMsg meetingMsg) {
        WebRTCManager.getInstance().sendMsg(meetingMsg);
    }

    public static void sendId(Activity mainActivity,  String ws, String sendId) {

        if (TextUtils.isEmpty(ws)) {
            ws = WSS;
        }

        WebRTCManager.getInstance().init(ws, iceServers, new IConnectEvent() {
            @Override
            public void onSuccess() {
//                ChatRoomActivity.openActivity(activity);
//               System.out.println("成功");

                WebRTCManager.getInstance().sendId(sendId);
            }
    ...
    public static void sendChatRequest(Activity activity, String wss, String roomId) {
